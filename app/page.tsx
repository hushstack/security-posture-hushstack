  ...
  const handleScan = async (domain: string) => {
    ...
    try {
      const response = await fetch('/api/scan', {
        ...
        body: JSON.stringify({ domain, mode: scanMode, enableAI }),
      });
            ...
            >
              ...
              >
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold transition-all shadow-md hover:shadow-lg cursor-pointer"
                  style={{
                    backgroundColor: enableAI ? '#6366f1' : 'var(--background-elevated)',
                    border: `2px solid ${enableAI ? '#6366f1' : 'var(--border-default)'}`,
                    color: enableAI ? 'white' : 'var(--foreground)',
                    boxShadow: enableAI 
                ...
                >
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.456 2.456L21.75 6l-1.035.259a3.375 3.375 0 00-2.456 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423L16.5 16.5l.394 1.183a2.25 2.25 0 001.423 1.423L19.5 18.75l-1.183.394a2.25 2.25 0 00-1.423 1.423z" />
                  </svg>
                  <span>AI Analysis {enableAI ? 'On' : 'Off'}</span>
                  <span 
                    className="ml-1 w-2 h-2 rounded-full"
                    style={{ 
                      backgroundColor: enableAI ? '#22c55e' : 'var(--foreground-muted)',
                      boxShadow: enableAI ? '0 0 8px #22c55e' : 'none'
                    }}
                  />
                </motion.button>
              </motion.div>
              ...
              >
                {[
                  ...
                  <motion.button
                    ...
                    whileHover={{ scale: 1.03, y: -2 }}
                    whileTap={{ scale: 0.97 }}
                    className="relative px-4 sm:px-5 py-3 rounded-xl font-semibold text-sm transition-all flex items-center gap-2 border-2 shadow-md hover:shadow-xl cursor-pointer"
                    style={{
                      backgroundColor: scanMode === mode.id ? mode.accent : 'var(--background-elevated)',
                      borderColor: scanMode === mode.id ? mode.accent : 'var(--border-default)',
                      color: scanMode === mode.id ? 'white' : 'var(--foreground)',
                      boxShadow: scanMode === mode.id 
                        ? `0 4px 20px ${mode.accent}50, inset 0 1px 0 rgba(255,255,255,0.2)` 
